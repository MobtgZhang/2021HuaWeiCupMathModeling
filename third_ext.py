...
def lgb_classifiy_train(args):
    ...
    # determine inputs dtype
    value_mol_file = os.path.join(args.raw_path, "Molecular_Descriptor.xlsx")
    root_path = os.path.join(args.log_path, "third_ext")
    admet_file = os.path.join(args.raw_path, "ADMET.xlsx")
    admet_mat_train = pd.read_excel(admet_file, sheet_name="training")
    admet_mat_test = pd.read_excel(admet_file, sheet_name="test")
    x_values = pd.read_excel(value_mol_file, sheet_name="training")
    ...
    params = {'task': 'train',
              ...
              'bagging_freq': 17}
    kf = KFold(n_splits=5)
    for k,(train_ids, val_ids) in enumerate(kf.split(X=x_values)):
        for raw_name in names_list:
            y_values = admet_mat_train[raw_name]

            train_x = x_values.iloc[train_ids, 1:]
            value_names = train_x.columns.values
            validate_x = x_values.iloc[val_ids, 1:].values
            train_y = y_values.iloc[train_ids].values
            validate_y = y_values.iloc[val_ids].values
        ...
        save_test_result = os.path.join(root_path, "lgbm_classifier_result_%d.xlsx"%k)
        pd.DataFrame(all_result).to_excel(save_test_result, index=None,header=None)
